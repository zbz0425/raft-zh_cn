...
* 如果`commitIndex > lastApplied`，则 lastApplied 递增，并将`log[lastApplied]`应用到状态机中（5.3 节）。个人理解：为了保证状态机的数据与已经提交的日志相匹配，提交日志的意义就是让日志生效。如果不应用提交的日志,则提交的意义被破坏。应用是提交的必然结果。
