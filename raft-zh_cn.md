...
| 参数 | 解释 |
| --- | --- |
| commitIndex | 已知已提交的最高的日志条目的索引（初始值为0，单调递增）,已知已提交的最高日志条目的索引值|
| lastApplied | 已经被应用到状态机的最高的日志条目的索引（初始值为0，单调递增）,已被应用到状态机的最高日志条目的索引值|
...
| lastLogIndex | 候选人的最后日志条目的索引值 , 仅表示日志中的最后一条|
...
2. 如果候选人的日志至少和自己一样新(lastLogTerm和lastLogIndex都大于等于接收者最新日志)并且接收者日志中 votedFor 为空或者为 candidateId，那么就投票给他（5.2 节，5.4 节）<br/>
个人理解：管理复制日志的一致性算法，所以只要候选人当选能够保持日志的一致性，那么就可以投票给当前候选人。
所以 1，任期比当前小肯定不允许，2，任期至少一样，如果大多数节点投票给这个候选人，那么可以保证后续日志一致性，日志至少和当前节点一样，那么当前节点就可以投票给这个候选人。

**所有服务器需遵守的规则**：

所有服务器：

* 如果`commitIndex > lastApplied`，则 lastApplied 递增，并将`log[lastApplied]`应用到状态机中（5.3 节）。<br>
  个人理解：为了保证状态机的数据与已经提交的日志相匹配，提交日志的意义就是让日志生效。如果不应用提交的日志,则提交的意义被破坏。应用是提交的必然结果。
  在一台 Raft 服务器节点上,lastApplied 和 commitIndex 之间的差距理想情况不会超过 1,原因如下:
  Raft 算法要求服务器节点必须先复制日志,经过多数服务器确认后才能提交(增加 commitIndex)。
  一次只提交 1 条日志,所以提交和应用之间最多差 1 条日志，服务器收到日志后会尽快应用,以保证状态机与提交日志一致。
  但是也存在一些情况会使差距增加:网络延迟导致复制和提交速度很快,而应用速度跟不上。服务器负载高,应用日志的速度很慢。
  以如果复制提交速度明显快于应用速度,lastApplied 和 commitIndex 的差距可能会暂时增加,这种情况下需要排查应用日志的瓶颈所在,比如优化应用日志的代码,增加应用性能等。
  但通常情况下,我们会尽量保证应用速度 matches 提交速度,这样差距在 1 以内就可以了。太大的差距会增加日志占用空间,也影响故障恢复时间。
...
* 响应来自候选人和领导人的请求
* 如果在超过选举超时时间的情况之前没有收到**当前领导人**（即该领导人的任期需与这个跟随者的当前任期相同）的心跳/附加日志，或者是给某个候选人投了票，就自己变成候选人<br/><br/>
  > 个人理解上面的原翻译有点不清楚，像是英文直译没有转换中文语法，应该是两种情况：<br/>
  > 1、以上次收到**当前领导人**（即该领导人的任期需与这个跟随者的当前任期相同）的心跳/附加日志开始计算在超过选举超时时间，自己就会变成候选人。<br/>
  > 2、收到候选人的投票请求并返回响应开始计算在超过选举超时时间，自己就会变成候选人。<br/>
...
* 如果接收到来自新的领导人的附加日志（AppendEntries）RPC，则转变成跟随者<br/>
  (包含心跳消息，心跳消息实际上是一种空内容的日志，本质上只有投票请求消息和日志消息)<br/>
* 如果选举过程超时，则再次发起一轮选举<br/>
  > 候选人发送投票请求后，只要收到两种消息就不会发生超时，从而发起新一轮选举。<br/>
  > 只要在心跳超时事件内收到两种消息就不会发生选举超时:<br/>
  > 1、一种是跟随者 follower 返回的投票响应。<br/>
  > 2、一种是新 Leader 发给候选人的心跳消息。<br/>
  > 心跳超时时间保证了选举可以再次进行。<br/>
...
>   4.1 如果 F跟随者 返回成功,则把 F跟随者 的 nextIndex 和 matchIndex 更新为最新值。<br/>
>   4.2 如果 F跟随者 返回失败,则减小 nextIndex 后重试,直到复制成功。<br/>
...
|领导人只附加原则| 领导人绝对不会删除或者覆盖自己的日志，只会增加（5.3 节）<br/>非领导人可能会被覆盖日志例如节点原来是领导人本机写入日志，没有收到大多数他节点都写入日志的响应，就宕机了，重新恢复后，变成跟随者之后会被新领导人的日志覆盖|
...
扩展新增部分：超时时间：
Raft算法中，有三种很重要的超时设置：<br>
（1）选举超时。就是新一轮选举开始时，每个节点随机思考要不要做领导者的时间，这个时间一般100-到200ms，非常短。<br>
假设集群由3个节点组成，为了防止3个节点同时发起投票，Raft会给每个节点分配一个随机的选举超时时间（Election Timeout）。在这个时间内，节点必须等待，不能成为Candidate状态。<br/>

现在假设节点a等待168ms，节点b等待210ms，节点c等待200ms。由于a的等待时间最短，所以它会最先成为Candidate，并向另外两个节点发起投票请求，希望它们能选举自己为Leader。另外两个节点收到请求后，假设将它们的投票返回给Candidate状态节点a，节点a由于得到了大多数节点的投票，就会从Candidate变为Leader。<br/>

（2）最小选举超时。在分布式系统中，有时候需要对集群中的成员数量进行更新的操作。对于被删除的服务器而言，如果它们没有及时关闭，那么它们将不会接收到心跳信息和日志信息，从而不断发生超时，最后导致任期不断增加（高于集群中所有成员的任期），然后不断向集群中发送请求投票消息。集群中的Leader将变为Follower，集群中将不断开始新的选举，从而扰乱集群的正常运行。<br/>

解决方案：Raft引入了一个最小选举超时时间，意思是如果集群中存在Leader时，并且接收到心跳信息之后在最小选举超时时间内接受到请求投票消息，那么将会忽略掉该投票消息。<br/>

（3）心跳超时。心跳是领导者发送心跳给跟踪者的时间。这个时间一旦超时，跟随者会觉得Leader挂了，这样会开始新的选举周期。<br/>

在 Raft 算法中,最小选举超时时间必须大于选举超时时间。<br/>
Raft 中有两个与选举相关的超时时间:<br/>
- 选举超时时间(election timeout):follower 在这个时间内没有收到 leader 的心跳,会变成候选人并发起新一轮选举。<br/>
- 最小选举超时时间(minimum election timeout):生成随机选举超时时间的下限。<br/>
其中最小选举超时时间必须严格大于选举超时时间。这主要有两个考虑:<br/>
1. 避免多数节点同时选举导致分裂。如果最小超时时间等于选举超时,当leader宕机时,多数follower同时变成候选人,可能会导致分裂。<br/>
2. 降低选举频率。最小超时时间大于选举超时可以减少多次不必要的选举。<br/>
通常情况下,会将最小选举超时设置为比选举超时多几十或几百毫秒,这样可以提供一定的随机间隔,避免大面积的同时选举,而且也不会延长领导人失败到恢复的时间。<br/>
所以 Raft 的最小选举超时时间必须设置得严格大于基本选举超时时间,这个设定对算法的稳定性和效率很重要。<br/>

心跳超时时间通常会设置得比选举超时时间要小,以此来触发新的选举:<br/>
- 心跳超时时间(heartbeat timeout):leader 需要向 follower 发送心跳的间隔时长。<br/>
- 选举超时时间(election timeout):follower 在这个时间内没有收到leader心跳,会变成候选人并发起新一轮选举。<br/>
Leader 会按照心跳超时时间周期性地向follower发送心跳信息。如果follower在选举超时内没有收到心跳,就会进入选举状态。<br/>
为了触发选举,心跳超时时间必须小于选举超时时间。<br/>
因此,通常的设置关系是:<br/>
心跳超时 < 选举超时 < 最小选举超时<br/>
举例如下:<br/>
- 心跳超时时间:50ms<br/>
- 选举超时时间:150ms<br/>
- 最小选举超时时间:300ms<br/>
这样就可以通过心跳超时来及时检测leader失败并触发选举,而选举超时时间还有一定的缓冲间隔,可以等待更多的选票信息。最小选举超时又大于选举超时,可以防止出现脑裂。<br/>
所以心跳超时要小于选举超时,这对快速检测领导人故障和触发选举非常重要。<br/>
